...
class NewsCubit extends Cubit<NewsStates> {
  ...
  void getApi(category) {
    ...
    DioHelper.getData(url: '/v2/top-headlines', query: {
      'country': 'us',
      'category': category,
      'apiKey': 'cdcd683e703448b08682a75878dc8f4a',
    }).then((value) {
      if (category == 'business' && business.length == 0) {
        business = value.data['articles'];
        refresh = true;
      } else if (category == 'sports' && sports.length == 0) {
        sports = value.data['articles'];
        refresh = true;
      } else if (category == 'science' && science.length == 0) {
